...
export default function Dashboard() {
  ...
  async function fetchUserStats() {
    ...
    try {
      ...
      const verifiedCount = allRequests.filter((r: any) => r.status === "verified").length;
      const activeLeadsCount = allRequests.filter((r: any) => r.status === "in_review" || r.status === "pending").length;
      // Count recently verified (last 7 days)
      const sevenDaysAgo = new Date();
      sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
      const recentlyVerifiedCount = allRequests.filter((r: any) => r.status === "verified" && new Date(r.updated_at) > sevenDaysAgo).length;
